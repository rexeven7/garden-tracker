...
export default function BedShape({
  ...
  // Resize handle positions: corners + edges (8 handles total)
  const handles = selected && !readOnly ? [
    // Corners
    { id: 'tl', cx: 0,   cy: 0   },
    { id: 'tr', cx: w,   cy: 0   },
    { id: 'bl', cx: 0,   cy: h   },
  ...
  const statusColors = {
    planned: '#94a3b8', seeded: '#60a5fa', transplanted: '#34d399',
    growing: '#4ade80', harvested: '#fbbf24', failed: '#f87171',
  }
  ...
  return (
    <Group
      ...
      onTap={() => onSelect?.(bed)}
      onMouseEnter={e => { if (!readOnly) e.target.getStage().container().style.cursor = 'move'; onHover?.(bed, e) }}
      onMouseLeave={e => { e.target.getStage().container().style.cursor = 'default'; onHoverEnd?.() }}
    >
      {/* Bed fill */}
      <Rect
        width={w}
      ...
      {/* Status overlay — when status layer is on */}
      {layers?.status && bed.plantings?.length > 0 && (() => {
        const statusPriority = ['failed', 'harvested', 'growing', 'transplanted', 'seeded', 'planned']
        const worstStatus = statusPriority.find(s => bed.plantings.some(p => p.status === s))
        const colors = { planned: '#94a3b830', seeded: '#60a5fa30', transplanted: '#34d39930', growing: '#4ade8030', harvested: '#fbbf2430', failed: '#f8717130' }
        return worstStatus ? (
          <Rect width={w} height={h} fill={colors[worstStatus] || 'transparent'} cornerRadius={4} listening={false} />
        ) : null
      })()}
      ...
      {/* Tasks badge — when tasks layer is on */}
      {layers?.tasks && (() => {
      ...
      {/* Plants layer rendered as children */}
      {children}
    </Group>
  )
}
